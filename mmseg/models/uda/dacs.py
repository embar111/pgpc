...
class DACS(UDADecorator):
    ...
    def transform_inputs(self, inputs):
        ...
        inputs = [inputs[i] for i in [0, 1, 2, 3]]
        upsampled_inputs = [
            resize(
                input=x,
    ...
    def forward_train(self,
        ...
        clean_losses = self.get_model().forward_train(
            img, img_metas, gt_semantic_seg, return_feat=True, return_fused=False)
        ...
        src_feat = self.transform_inputs(src_feat)
        src_feat = F.interpolate(src_feat, size = fm_size,
                                     mode = 'bilinear', align_corners=True)
        lr_gt = F.interpolate(gt_semantic_seg.float(), size = fm_size,
                           mode = 'nearest').long()
        src_cl = contrastive_loss(src_feat,
                                  lr_gt,
                                 fdist_classes=self.fdist_classes)
        src_res_cl_loss, src_cl_log = \
            self._parse_losses({'src.explicit_cl_loss': src_cl})
        encoder_loss = src_cl + feat_loss
        encoder_loss.backward()
        log_vars.update(src_cl_log)

        del feat_loss, src_res_cl_loss
        ...
        pseudo_weight = torch.sum(ps_large_p).item() / ps_size * self.lambda_t
        ...
        mix_feat = self.transform_inputs(mix_feat)
        mix_feat = F.interpolate(mix_feat, size=fm_size,
                                 mode='bilinear', align_corners=True)
        ...
        mix_cl = contrastive_loss(mix_feat,
                                mix_lbl,
                                pseudo_weight=cl_pseudo_weight,
                                fdist_classes=self.fdist_classes)
        mix_multi_res_cl_loss, mix_cl_log = self._parse_losses({'mix.explicit_cl_loss': mix_cl})
        ...
        del mix_lbl, mix_multi_res_cl_loss
